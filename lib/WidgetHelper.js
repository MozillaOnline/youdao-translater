...
let RETINA_STYLE_TEMPLATE =
  '  @media (min-resolution: 2dppx) {\n' +
  // Make indention right.
  STYLE_TEMPLATE.replace(/^\s+/mg, '$&  ') +
  // Set width right.
  '    #__id__ > .toolbarbutton-icon,\n' +
  '    #__id__ > .toolbarbutton-menubutton-button > .toolbarbutton-icon {\n' +
  '      width: 16px;\n' +
  '    }\n' +
  '  }\n';
...
let onCommand = null;
let currentStatus = '';
let isMac = platform.toLowerCase() == 'darwin';
let isYosemite = (Services.vc.compare(Services.appinfo.version, "34.0") >= 0) && (Number(Services.sysinfo.getProperty('version').split('.')[0]) >= 14);
...
function getStyleSheet() {
  ...
  function getIcon(status, themeStyle, size, retina, active) {
    return data.url(themeStyle + '/' +
                    // Pick icons for mac.
                    (isMac ? (isYosemite ? 'mac/yosemite/' : 'mac/') : '') +
                    buttonIcons[status] + '-' + size +
                    // Only mac has :active styles.
                    (isMac && active ? '-active' : '') +
                    // @2x suffix is for retina display.
                    (retina ? '@2x' : '') +
                    '.png');
  }

  function genStyle(retina) {
    let template = retina ? RETINA_STYLE_TEMPLATE : STYLE_TEMPLATE;
    // Generate style rules for each status.
    Object.keys(buttonIcons).forEach(function(status) {
      let icons = buttonIcons[status];
      let s = template.replace(/__id__/g, buttonId).
        replace(/__status__/g, status == 'default' ? '' : '[data-status="' + status + '"]').
        replace(/__icon__brightTheme__/g, getIcon(status, 'brighttheme', 16, retina)).
        replace(/__icon__active__brightTheme__/g, getIcon(status, 'brighttheme', 16, retina, true)).
        replace(/__icon__darkTheme__/g, getIcon(status, 'darktheme', 16, retina)).
        replace(/__icon__active__darkTheme__/g, getIcon(status, 'darktheme', 16, retina, true)).
        replace(/__icon__32__brightTheme__/g, getIcon(status, 'brighttheme', 32, retina));
      styles.push(s);
    });
  }

  styles.push(genStyle());
  if (isMac) {
    // Generate style rules for retina.
    styles.push(genStyle(true));
  }
