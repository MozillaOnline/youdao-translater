...
function showPanelWithButton(p) {
  ...
  // The anchor position is wrong in FF29, but OK in 30+
  if (xulApp.versionInRange(xulApp.platformVersion, "30.0*", "*")) {
    ...
    p.show(win.document.getElementById(buttonId));
    return;
  }
  ...
  switch (areaType) {
    case 'menu-panel':
      button = win.document.getElementById('PanelUI-menu-button');
      break;
    case 'toolbar':
      if (CustomizableUI.getWidget(buttonId).forWindow(win).overflowed) {
        button = win.document.getElementById('nav-bar-overflow-button');
      }
      break;
  }
