...
function getStyleSheet() {
  ...
  Object.keys(buttonIcons).forEach(function(status) {
    ...
    status = status == 'default' ? '' : '[data-status="' + status + '"]';
    let s = STYLE_TEMPLATE.replace(/__id__/g, buttonId).
      replace(/__status__/g, status).
      replace(/__icon__brightTheme__/g, icons.brightTheme['16']).
      replace(/__icon__darkTheme__/g, icons.darkTheme['16']).
      replace(/__icon__32__brightTheme__/g, icons.brightTheme['32']);
    styles.push(s);
  });
...
exports.init = function({id, oncommand, icons, label, area}) {
  ...
  Object.keys(icons).forEach(function(k) {
    let ics = icons[k];
    if (!ics.brightTheme || !ics.brightTheme['16'] || !ics.brightTheme['32'] ||
        !ics.darkTheme || !ics.darkTheme['16']) {
      throw new Error('Missing some icons.');
    }
  });
